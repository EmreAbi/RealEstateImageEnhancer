-- Auto-create profile when a new user signs up
-- Bu script Supabase Dashboard > SQL Editor'de çalıştırılmalıdır

-- Function to automatically create profile
CREATE OR REPLACE FUNCTION public.handle_new_user()
RETURNS TRIGGER AS $$
BEGIN
  INSERT INTO public.profiles (id, username, real_estate_office, email)
  VALUES (
    NEW.id,
    COALESCE(NEW.raw_user_meta_data->>'username', 'User'),
    COALESCE(NEW.raw_user_meta_data->>'real_estate_office', 'Real Estate Office'),
    NEW.email
  );
  RETURN NEW;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

-- Trigger to call the function after user signup
DROP TRIGGER IF EXISTS on_auth_user_created ON auth.users;
CREATE TRIGGER on_auth_user_created
  AFTER INSERT ON auth.users
  FOR EACH ROW EXECUTE FUNCTION public.handle_new_user();

-- Update RLS policy for INSERT to allow the trigger
-- Remove the old insert policy
DROP POLICY IF EXISTS "Users can insert their own profile" ON profiles;

-- Create new policy that allows insert via trigger (using SECURITY DEFINER)
CREATE POLICY "Enable insert for authenticated users during signup"
  ON profiles FOR INSERT
  WITH CHECK (true);

-- Keep other policies as they are
-- Users can still only view and update their own profile
